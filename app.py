...
def data_process(month_name, year):
    # path = "C:/Sonali/ISB/Term5/FP2/Code/"
    existing_data1 = pd.read_csv("data/generation4_data.csv")
...
def output():
    if request.method == "POST":
        ...
        max_demand, total_consumption = get_maximum_demand( month, year)
        ...
        #Read models
        tpot_hydro_gen_model = joblib.load('tpot_model/tpot_hydro_gen_model.pkl')
        tpot_solar_gen_model = joblib.load('tpot_model/tpot_solar_gen_model.pkl')
        tpot_thermal_gen_model = joblib.load('tpot_model/tpot_thermal_gen_model.pkl')
        tpot_wind_gen_model = joblib.load('tpot_model/tpot_wind_gen_model.pkl')
        ...
        total = hydro + solar + thermal + wind + int(watt)
        print(total)

        total = round(total, 2)

        return render_template("output.html", hydro = hydro, solar = solar, thermal = thermal, wind = wind, total = total, max_demand=max_demand, total_consumption=total_consumption)
